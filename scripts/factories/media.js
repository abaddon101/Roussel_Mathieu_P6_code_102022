...
function mediaFactory(media, photographer) {
  ...
  function getMediaCardDOM() {
    ...
    if (isClicked !== true) {
      heartLikes.innerHTML = `<i tabindex="0" class="heartLike fa-regular fa-heart" data-id=" ${media.id}"></i>`;
    } else {
      heartLikes.innerHTML = `<i tabindex="0" class="heartLike fa-solid fa-heart" data-id=" ${media.id}"></i>`;
    }
    return article;
  }
  return {
