...
function mediaFactory(media, photographer) {
  const {
    photographerId, title, image, video, likes, date, isClicked,
  } = media;
  const {
    city, country, name, tagline, portrait, price, id,
  } = photographer;
  ...
  function getMediaCardDOM() {
    ...
    if (isClicked !== true) {
      heartLikes.innerHTML = `<i tabindex="0" class="heartLike fa-regular fa-heart" data-id=" ${
        media.id
      }"></i>`;
    } else {
      heartLikes.innerHTML = `<i tabindex="0" class="heartLike fa-solid fa-heart" data-id=" ${
        media.id
      }"></i>`;
    }
    return article;
